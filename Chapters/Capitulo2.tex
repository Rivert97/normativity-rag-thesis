...
codificador-decodificador, es decir, una red neuronal dividida en dos partes
...
en tareas de distintos dominios, moderar sus respuestas,
...
\cite{shazeer_outrageously_2017} y empleada en \textit{transformers} por Du et al. \cite{du_glam_2022}. Esta arquitectura
...
los documentos, aunque sí deben estar en formato de texto.
Estos documentos se separan en fragmentos, comúnmente
...
representación vectorial (\textit{embedding}) de cada fragmento, para finalmente
almacenar cada uno con su respectivo \textit{embedding} en una base
de datos.

\subsection{Recuperación}

Cuando se le hace una solicitud al LLM, la recuperación consiste
en buscar información relacionada, en forma de fragmentos de
documentos, en la base de datos previamente construida. Para determinar
cuáles son los fragmentos que más se relacionan con la pregunta, se
calcula la similitud semántica entre la pregunta y cada fragmento.
Una vez calculada la similitud con todos los fragmentos disponibles, se
seleccionan los \textit{k} fragmentos más similares a la pregunta o se
establece un valor umbral y se consideran como relacionados a aquellos
que estén por encima del umbral. Uno de los cálculos de distancia
más comunes es la distancia coseno, en su forma de similitud coseno,
como se muestra en la Ecuación \ref{eq:cos_similarity}.
...
En esta etapa, la solicitud y los fragmentos marcados como relacionados
son sintetizados en una instrucción coherente que se le proporciona al modelo.
...
el modelo pre-entrenado y solo entrena un pequeño número de parámetros adicionales.

\subsection{Aprendizaje por refuerzo}

El aprendizaje por refuerzo consiste en entrenar al modelo para tomar decisiones
que maximicen una recompensa. En el caso de los modelos de lenguaje, uno
...
metodología RAG y para referenciar las respuestas del sistema.
...
Uno de los retos principales del RAG es la recuperación adecuada de fragmentos
relacionados con las preguntas. Trabajos como el de Shao et al. \cite{shao_enhancing_2023}
se centran en proponer mejores técnicas en la identificación de
fragmentos relacionados en grandes volúmenes de datos, aplicando una
...
fino para el modelo que obtiene los \textit{embeddings} de los fragmentos
